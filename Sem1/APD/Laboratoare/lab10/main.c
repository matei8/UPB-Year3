...
int* get_dst(int rank, int numProcs, int leader) {
	...
	*/
    if (rank != leader) {
        ...
        MPI_Recv(v, numProcs, MPI_INT, v[rank], TAG_ECOU, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
...
int leader_chosing(int rank, int nProcesses) {
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
        ...
        for (int i = 0; i < num_neigh; i++) {
            int received_leader;
            MPI_Recv(&received_leader, 1, MPI_INT, MPI_ANY_SOURCE, TAG_SONDA, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
int get_number_of_nodes(int rank, int leader) {
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
        ...
        for (int i = 0; i < num_neigh; i++) {
            double received_val;
            MPI_Recv(&received_val, 1, MPI_DOUBLE, MPI_ANY_SOURCE, TAG_SONDA, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            val = (val + received_val) / 2;
        }
	}
...
int ** get_topology(int rank, int nProcesses, int * parents, int leader) {
    ...
    for (int i = 0; i < num_neigh; i++) {
        MPI_Recv(vTopology[neigh[i]], nProcesses, MPI_INT, neigh[i], TAG_ECOU, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        for (int j = 0; j < nProcesses; j++) {
            if (vTopology[neigh[i]][j] == 1) {
                topology[neigh[i]][j] = 1;
            }
        }
    }
    ...
    if (rank != leader) {
        MPI_Send(topology[rank], nProcesses, MPI_INT, parents[rank], TAG_ECOU, MPI_COMM_WORLD);
    }

	/* TODO4: Daca nu suntem liderul, asteptam topologia completa de la parinte  */
    if (rank != leader) {
        MPI_Recv(topology[rank], nProcesses, MPI_INT, parents[rank], TAG_ECOU, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
	
	/* TODO4: Trimitem topologia completa copiilor */
    if (rank == leader) {
        for (int i = 0; i < num_neigh; i++) {
            MPI_Send(topology[rank], nProcesses, MPI_INT, neigh[i], TAG_ECOU, MPI_COMM_WORLD);
        }
    }
